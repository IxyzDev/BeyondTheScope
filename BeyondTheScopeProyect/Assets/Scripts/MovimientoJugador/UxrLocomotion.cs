...
namespace UltimateXR.Locomotion
{
    ...
    /// </summary>
    public class UxrSLocomotion : UxrLocomotion
    {
        ...
        [SerializeField] private UxrHandSide _upButtonHand = UxrHandSide.Right; // Mano para el botón de subir
        [SerializeField] private UxrInputButtons _upButton = UxrInputButtons.Joystick; // Botón para subir
        [SerializeField] private UxrHandSide _downButtonHand = UxrHandSide.Right; // Mano para el botón de bajar
        [SerializeField] private UxrInputButtons _downButton = UxrInputButtons.Joystick; // Botón para bajar
        [SerializeField] private float _metersPerSecondVertical = 1.0f; // Velocidad vertical normal
        [SerializeField] private float _metersPerSecondVerticalSprint = 2.0f; // Velocidad vertical rápida (sprint)
        ...
        protected override void UpdateLocomotion()
        {
            if (Avatar)
            {
                ...
                bool isMovingUp = Avatar.ControllerInput.GetButtonsPress(_upButtonHand, _upButton);
                bool isMovingDown = Avatar.ControllerInput.GetButtonsPress(_downButtonHand, _downButton);
                ...
                if (isMovingUp == isMovingDown) // Evita subir y bajar al mismo tiempo
                {
                    _verticalSpeed = 0.0f;
                }
                else if (isMovingUp)
                {
                    _verticalSpeed = (isSprinting ? _metersPerSecondVerticalSprint : _metersPerSecondVertical);
                }
        ...
        private void UpdateLocomotionPhysics(float deltaTime)
        {
            ...
            {
                Vector3 verticalMovement = Vector3.up * _verticalSpeed * deltaTime;
                UxrManager.Instance.TranslateAvatar(Avatar, verticalMovement);
            }
        ...
        private float _verticalSpeed; // Velocidad vertical actual
